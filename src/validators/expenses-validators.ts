...
import { AddExpenseRequestBody, UpdateExpenseRequestBody } from '@/types/expense-types'
...
export const validateExpenseId = ({
  expenseId,
}: Partial<{ expenseId: string }>): ValidationResult<{ expenseId: string }> => {
  const validationResult = validateExpenseIdSchema.safeParse({ expenseId })
...
export const validateUpdateExpense = (
  body: Partial<UpdateExpenseRequestBody>,
): ValidationResult<Omit<UpdateExpenseRequestBody, 'date'> & { date?: Date }> => {
